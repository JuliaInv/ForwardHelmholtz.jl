...
if formulation == 1 # Pure Helmholtz
	...
	if eltype(q)==Complex64
		Shift = convert(SparseMatrixCSC{Complex64,Int32},Shift);
	end
	...
	if eltype(q)==Complex64
		HT = convert(SparseMatrixCSC{Complex64,Int32},HT);
	end
...
elseif formulation == 4 # ADR long up
	...
	ADR_longUp = ADR_longUp';
	if eltype(q)==Complex64
		ADR_short = convert(SparseMatrixCSC{Complex64,Int32},0.25*ADR_short') + convert(SparseMatrixCSC{Complex64,Int32},0.75*ADR_longUp);
	else
